...
<form id="form1" name="form1" method="post"
		action='<c:url value='/t6_10_doUpdateUser.controller' />' enctype="multipart/form-data">
...
<%--                     <td><input type="checkbox" name="active" value="${user.active}"></td> --%>
<%--                     <td><input type="password" name="password"  style="border:0px;background:rgba(0, 0, 0, 0);text-align:center;"  value="${account.password}" required></td> --%>
               		<td><input type='email' name='email' placeholder='請輸入Email' value="${user.email}" required/></td>
                    <td>
											<img id="preImg" height='100' width='80' 
											src="<c:url value='/t6_10_UserImg.controller/${user.id}' />"><input id="uploadImg" type='file' name='photo' accept='.png,.jpg'/></td>
<!-- 				<td><button type="submit" value="upload" class="insert">修改	</button></td> -->
					<td>
					<button class="btn btn-success insert">
